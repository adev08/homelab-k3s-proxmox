...
if [[ ! "$response" =~ ^[Yy][Ee][Ss]$ ]]; then
    echo -e "${RED}Deployment caNoColorelled.${NoColor}"
    exit 0
fi
...
until ssh -o StrictHostKeyChecking=no -o ConnectTimeout=5 "ubuntu@${CONTROL_PLANE_IP}" "echo 'SSH OK'" &> /dev/null; do
    ...
    if [ $retries -ge $max_retries ]; then
        echo -e "${RED}Failed to connect via SSH after ${max_retries} attempts${NoColor}"
        exit 1
    fi
...
echo -e "${GREEN}SSH connectivity confirmed!${NoColor}"
...
echo -e "\n${YELLOW}Do you want to install ArgoCD? (yes/no)${NoColor}"
read -r response
if [[ "$response" =~ ^[Yy][Ee][Ss]$ ]]; then
...
echo -e "\n${GREEN}================================${NoColor}"
echo -e "${GREEN}Deployment Complete!${NoColor}"
echo -e "${GREEN}================================${NoColor}"
